...
public class MyView extends View {
    ...
    private int m_nCurrentPlayer = 1; // 1 : player MAX, -1 : player MIN, 0: empty
    ...
    private void initTable() {
        ...
        listMove = new ArrayList<Point>();

        computer = new Computer(m_matrixState, m_nRow, m_nCol, listMove);
        m_nCurrentPlayer = 1;
        resetMatrixState();
    }
        ...
        try {
            ...
            for (int row = 0; row < m_nRow; row++)
                for (int col = 0; col < m_nCol; col++) {
                    ...
                    } else if (nState == Computer.PLAYER_MIN) {
                        m_rectCell.offsetTo(col * m_nCellSize + MARGIN, row * m_nCellSize + MARGIN);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (nCurX <= m_rectTable.right && nCurX >= m_rectTable.left && nCurY <= m_rectTable.bottom && nCurY >= m_rectTable.top) {
                ...
                if (m_matrixState[nRowIndex][nColIndex] != Computer.EMPTY_CELL) {
                    return true;
                }

                if (m_nCurrentPlayer == Computer.PLAYER_MAX) {
                    m_matrixState[nRowIndex][nColIndex] = Computer.PLAYER_MAX;
                    m_nCurrentPlayer = Computer.PLAYER_MIN;
                    listMove.add(new Point(nRowIndex, nColIndex));
                }

                Move mv = computer.computerPlay(m_nCurrentPlayer);
                Point p = new Point(mv.getRow(), mv.getCol());
//                    Point p = ComputerPlay(m_matrixState, m_nRow, m_nCol);
                listMove.add(p);

                m_matrixState[p.x][p.y] = Computer.PLAYER_MIN;
                m_nCurrentPlayer = Computer.PLAYER_MAX;

                if (TestGameFinish(m_matrixState, m_nRow, m_nCol)) {
                    resetMatrixState();
                    m_nCurrentPlayer = Computer.PLAYER_MAX;
                    listMove.clear();
                }
            ...
            for (int j = 0; j < column; j++)
                if (chessBoard[i][j] == 0) // chua co quan co
                {
                    ...
                    if (ptx > tmp) {
                        ...
                        if (tmp >= 10000)
                            return t;
                    } else if (ptx == tmp && rd.nextInt(2) == 0) {
                        t.x = i;
                        t.y = j;
                    }
                }
    ...
    private int evaluate(int[][] chessBoard, int i, int j, int row, int column) {
        ...
        chessBoard[i][j] = Computer.PLAYER_MIN;
        ...
        chessBoard[i][j] = Computer.PLAYER_MAX;
        for (int h = 0; h < 4; h++) {
            Point p, q;
            p = runline(chessBoard, row, column, i, j, dx[h], dy[h]);
            q = runline(chessBoard, row, column, i, j, (int) -dx[h], (int) -dy[h]);
